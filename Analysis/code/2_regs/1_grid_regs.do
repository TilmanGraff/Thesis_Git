...
foreach outcome in "Lambda" "amenity_pc" "amenity" "i_stat" {
	
	
	
	
foreach type in "imm" "mob" "imm_10p" "L_mob" "L_mob_10p" "L_mob_nocomp" "L_mob_nocomp_10p" "imm_nocomp" "imm_nocomp_10p"{
	...
	if "`type'" == "imm_nocomp"{
	  loc type_addon = "_imm_nocomp"
	  loc lambda_sub = "\text{imm}"
	  loc lambda_sup = ""
	}
	if "`type'" == "imm_nocomp_10p"{
	  loc type_addon = "_imm_nocomp_10p"
	  loc lambda_sub = "\text{imm}"
	  loc lambda_sup = "10\%"
	}
	if "`type'" == "L_mob_nocomp"{
	  loc type_addon = "_mob_nocomp"
	  loc lambda_sub = "\text{mob}"
	  loc lambda_sup = ""
	}
	if "`type'" == "L_mob_nocomp_10p"{
	  loc type_addon = "_mob_nocomp_10p"
	  loc lambda_sub = "\text{mob}"
	  loc lambda_sup = "10\%"
	}
	...
	if "`outcome'" == "amenity_pc"{
		loc title "Housing per capita (z-scored)"
	}
	if "`outcome'" == "amenity"{
		loc title "Housing (z-scored)"
	}
	if "`outcome'" == "Lambda"{
...
esttab modr* using "`outpath'/rails_`outcome'`sim_addon'`type_addon'.tex", se replace keep(railkm_50 placebokm_50 railkm_military_50 railkm_mining_50 dist10 dist20 dist30 dist40) stats(fe gctrls  N r2, labels("Country FE" "Geography Controls"  "N" "R2")) coeflabels(railkm_50 "50 KM of Colonial Railroads" placebokm_50 "50 KM of Colonial Placebo Railroads" railkm_military_50 "50 KM of Colonial Railroads for Military Purposes" railkm_mining_50 "50 KM of Colonial Railroads for Mining Purposes" dist10 "$<$10KM to railroad" dist20 "10-20KM to railroad" dist30 "20-30KM to railroad" dist40 "30-40KM to railroad") nomtitle collabels(none) mgroups("`title'" "Real" "Placebo", pattern(1 0 0 0 1 1)  ///
...
esttab moda* using "`outpath'/aid_`outcome'`sim_addon'`type_addon'.tex", se replace keep(`xvars') stats(fe gctrls  N r2, labels("Country FE" "Geography Controls"  "N" "R2")) coeflabels(dis "Total disbursements (mil \\$)" dis_transp "Total transport sector disbursements (mil \\$)" num "Number of projects" num_transp "Number of transport sector disbursements") nomtitle collabels(none) mgroups("$\Lambda$: World Bank" "$\Lambda$: China", pattern(1 0 0 0 1 0 0 0)  ///
prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) nonotes substitute(\_ _) star(* 0.1 ** 0.05 *** 0.01)
...
foreach type in "imm" "imm_10p" "L_mob" "L_mob_10p" "amenities" "amenities_pc"{
	...
	if "`type'" == "imm"{
		foreach var in `simctrls_balance' `geoctrls_balance'  "zi_stat_imm"{
			...
			loc dir_descr_sig "`dir_descr_sig'  `this_sig'"
		}
	}
...
foreach var in `simctrls_balance' `geoctrls_balance'  "zi_stat_imm" {
	...
	loc intct_vars "`intct_vars' `var' c.`var'##i.anyrail"
...
qui areg anyrail `simctrls_balance' `geoctrls_balance'  zi_stat_imm x* y*, a(ccode) cluster(cluster)
...
foreach var in `simctrls_balance' `geoctrls_balance'  "zi_stat_imm" {
	...
	local thisdir_sig : word `varcount' of `dir_descr_sig'
	...
	if `r(p)' < 0.05{
		...
		estadd local thisstat`count' = "\textbf{" + string(`r(p)', "%9.2f") + "}" : col3
	}
	else{
	 ...
	 areg anyrail `simctrls_balance' `geoctrls_balance'  zi_stat_imm x* y*, a(ccode) cluster(cluster)
	
	local pjointval = (2 * ttail(e(df_r), abs(_b[`var']/_se[`var'])))
	
	if `pjointval' < 0.05{
		loc mydir_sig = _b[`var']
		estadd local thisstat`count' ="\textbf{" + string(`pjointval', "%9.2f") + "}" : col4
	}
	else{
		loc mydir_sig = 0
		estadd local thisstat`count' = string(`pjointval', "%9.2f") : col4
	}
	
	loc mydir = _b[`var']
	
	local biasdir = `mydir' * `thisdir'
	local biasdir_sig = `mydir_sig' * `thisdir_sig'
	
	if `biasdir_sig' != 0{
		if `biasdir' > 0{
		estadd local thisstat`count' = "\textbf{Against}" : col5
		}
		if `biasdir' < 0{
			estadd local thisstat`count' = "\textbf{In favour}" : col5
		}
	}
	
	if `biasdir_sig' == 0{
// 		if `biasdir' < 0{
// 		estadd local thisstat`count' = "\emph{(Against)}" : col4
// 		}
// 		if `biasdir' > 0{
// 			estadd local thisstat`count' = "\emph{(In favour)}" : col4
// 		}
		estadd local thisstat`count' = "\emph{(unclear)}" : col5
// 		if `biasdir' < 0{
