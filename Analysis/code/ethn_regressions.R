...
############
#
# mod.1 <- lm(zeta~split5pc+factor(country), data=opt_acled)
# mod.2 <- lm(zeta~split5pc+factor(country)+log(km2split)+altitude+temp+landsuit+malaria+growingdays+precip+lakedum+riverdum+petroleum+diamondd+x+x_2+x_3+x_4+y+y_2+y_3+y_4, data=opt_acled)
# mod.3 <- lm(zeta~split5pc+factor(country)+log(km2split)+altitude+temp+landsuit+malaria+growingdays+precip+lakedum+riverdum+petroleum+diamondd+x+x_2+x_3+x_4+y+y_2+y_3+y_4+pop+lights+rugg+urban, data=opt_acled)
# mod.4 <- lm(zeta~all+split5pc+factor(country)+log(km2split)+altitude+temp+landsuit+malaria+growingdays+precip+lakedum+riverdum+petroleum+diamondd+x+x_2+x_3+x_4+y+y_2+y_3+y_4+pop+lights+rugg+urban, data=opt_acled)
# mod.5 <- lm(zeta~fatal+split5pc+factor(country)+log(km2split)+altitude+temp+landsuit+malaria+growingdays+precip+lakedum+riverdum+petroleum+diamondd+x+x_2+x_3+x_4+y+y_2+y_3+y_4+pop+lights+rugg+urban, data=opt_acled)
# mod.6 <- lm(zeta~riots+split5pc+factor(country)+log(km2split)+altitude+temp+landsuit+malaria+growingdays+precip+lakedum+riverdum+petroleum+diamondd+x+x_2+x_3+x_4+y+y_2+y_3+y_4+pop+lights+rugg+urban, data=opt_acled)
# mod.7 <- lm(zeta~vio+split5pc+factor(country)+log(km2split)+altitude+temp+landsuit+malaria+growingdays+precip+lakedum+riverdum+petroleum+diamondd+x+x_2+x_3+x_4+y+y_2+y_3+y_4+pop+lights+rugg+urban, data=opt_acled)

#covariate_labels <- c("years_in_power")
...
stargazer(mo_list, se=se_list, type="text",  keep = c("split5pc", "all", "vio", "riots", "power", "ail", "placebo", "KM", "ten", "thirty", "smaller", "zeta"), p.auto=TRUE, t.auto=TRUE, add.lines=control_list, keep.stat=c("n"))
