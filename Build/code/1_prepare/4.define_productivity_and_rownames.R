...
for (country in country_names){
  ...
  if(J >= N_eff){
    centroids_merged[centroids_merged$country==country, "good_produced"] <- pmin(rank(-centroids_merged[centroids_merged$country==country, "pop"]), N_eff) # assign goods 1 to (N-1) to the (N-1) biggest locations, and then assign good N to all other locations
