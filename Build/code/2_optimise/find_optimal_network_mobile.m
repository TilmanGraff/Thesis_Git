...
nums = nan(1);
 ...
 for countryID = 1:length(country_names)
     ...
     if num_locations > 2 && num_locations < 50
        ...
        param.Hj = population .* (1-alpha) .* ((res_stat.cj ./ alpha)).^(alpha / (alpha-1));
        res_stat2 = solve_allocation(param,g,I);

        
        param = init_parameters('Annealing', 'off', 'LaborMobility', 'partial', 'a', a, 'sigma', sigma, 'N', N, 'alpha', alpha, 'beta', beta, 'gamma', gamma, 'verbose', 'off', 'rho', rho, 'K', K);

        [param,g]=create_graph(param,[],[],'X',case_centroids.x,'Y',case_centroids.y,'Type','custom','Adjacency',adj,'X',case_centroids.x,'Y',case_centroids.y, 'NRegions', sum(case_centroids.abroad)+1);
        
        
        param.Lj = population + (population == 0) .* 10^-7; % this is needed as ipopt crashes otherwise (need to have population > 0 always), so I add an infinitisimal person
        param.Zjn = productivity;
        param.omegaj = weights;
        param.Hj = (1-alpha) .* population .* ((res_stat.cj ./ alpha)).^(alpha / (alpha-1));
        regions = cumsum(case_centroids.abroad);
        regions(case_centroids.abroad == 0) = (sum(case_centroids.abroad) + 1);
        param.regions = regions;

        param.omegar = [case_centroids.abroad(case_centroids.abroad == 1)-1; 1];
        param.Lr = [case_centroids.pop(case_centroids.abroad == 1); sum(case_centroids.pop(case_centroids.abroad == 0))];

        res_stat3 = solve_allocation(param,g,I);
