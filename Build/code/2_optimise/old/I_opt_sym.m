...
I_opt_sym_nofour = I_raw * 1/sum(sum(I_raw .* delta_I .* (adj-abr))); % this denominator is just = 1 if I_raw were already perfect
...
% Rescale the inland ones: Currently redundant
%I_opt_sym_replaceabroad = I_opt_sym_bbelow * 1/(sum(sum(I_opt_sym_bbelow .* (adj - abr) .* delta_I)));


% Bounding from Above

x = min(I_opt_sym_bbelow, ones(J).*120.*adj);

while sum(sum(x .* delta_I .* (adj-abr))) < 0.9999
    mu3 = 1 / sum(sum((min(x, ones(J) .* 120)) .* (adj-abr) .* delta_I)); % this is everything above 120
    x = min(x * mu3, ones(J).*120.*adj);
end


%I_opt_sym_babove = I_opt_sym_bbelow * mu3;

% Replace the abroad ones:
I_opt_sym = x .* (adj - abr) + I .* abr;
